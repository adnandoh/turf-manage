...
import { cricketService } from '../api/api';
...
const Cricket = () => {
  ...
  const fetchSlots = useCallback(async () => {
    console.log('🏏 CRICKET: fetchSlots called with date:', formattedDate);
    setLoading(true);
    try {
      console.log('🏏 CRICKET: Making API call to:', `/api/cricket/slots/?date=${formattedDate}`);
      
      // Fetch slots from API (admin gets all slots including blocked ones)
      const response = await cricketService.getSlots(formattedDate);
      console.log('🏏 CRICKET API Response:', response.data);
      console.log('🏏 CRICKET Auth token:', localStorage.getItem('auth_token') ? 'Present' : 'Missing');
      ...
      // Debug logging
      console.log('🏏 CRICKET Stats calculated:', {
        total,
        available,
        blocked,
        booked,
      ...
      console.error('🏏 CRICKET Error fetching slots:', error);
  ...
  useEffect(() => {
    console.log('🏏 CRICKET: Component mounted');
    return () => {
      console.log('🏏 CRICKET: Component unmounted');
    };
  }, []);
  ...
  const handleToggleBlock = async (slot, reason = 'Blocked by admin') => {
    try {
      if (slot.is_blocked) {
        // Unblock the slot
        await cricketService.removeBlock(slot.id);
        ...
        await cricketService.createBlock(blockData);
  ...
  const handleBlockDates = async (dates, reason) => {
    ...
    try {
      await cricketService.blockDates(dates, reason);
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          ...
          <Box sx={{ flex: 1 }}>
            <Typography variant="h4" sx={{ fontWeight: 700, mb: 0.5 }}>
              Cricket Court Management
            </Typography>
            <Typography variant="body1" color="text.secondary">
              Manage cricket bookings, slots, and availability for {format(currentDate, 'EEEE, MMMM d, yyyy')}
            </Typography>
          </Box>
          <Button
            variant="outlined"
